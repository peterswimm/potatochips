// A SunSoft FME7 chip algorithm for the disting NT.
...
enum {
    ...
    kParamOutput = OSC_COUNT * kNumToneParams,
...
#define TONE_PARAMETERS( NAME ) \
    { .name = NAME " coarse", .min = -54, .max = 54, .def = 0, .unit = kNT_unitSemitones, .scaling = 0, .enumStrings = NULL }, \
...
/// @brief The state of a Pulses instance.
struct _pulsesAlgorithm : public _NT_algorithm {
    /// @brief Initialize a new instance.
    _pulsesAlgorithm() : voice(VOLUME) { }
...
void calculateRequirements(
    ...
    req.sram = sizeof(_pulsesAlgorithm);
    req.dram = 0;
    req.dtc = 0;
    req.itc = 0;
}
...
_NT_algorithm* construct(
    ...
    _pulsesAlgorithm* alg = new (ptrs.sram) _pulsesAlgorithm();
...
void step(_NT_algorithm* self, float* busFrames, int numFramesBy4) {
    _pulsesAlgorithm* pThis = (_pulsesAlgorithm*)self;
    ...
    for (unsigned frame = 0; frame < numFrames; frame++) {
        // audio rate: the pitch of each oscillator, which occupies two
        // registers per voice
        for (unsigned osc = 0; osc < OSC_COUNT; osc++) {
            ...
            pThis->voice.apu.write(Chip::PULSE_A_LO + (osc << 1), freq & 0xff);
            pThis->voice.apu.write(Chip::PULSE_A_HI + (osc << 1), (freq >> 8) & 0x0f);
        }

        // control rate: the level of each oscillator
        if (pThis->voice.isControlRate()) {
            for (unsigned osc = 0; osc < OSC_COUNT; osc++) {
                // bit 4 selects a fixed level rather than the envelope
                pThis->voice.apu.write(
                    Chip::PULSE_A_ENV + osc,
                    0x10 | getVolume(
...
static const _NT_factory factory = {
    .guid = NT_MULTICHAR( 'P', 'C', 'f', '7' ),
    .name = "Pulses",
    .description = "SunSoft FME7 (Nintendo Entertainment System)",
    .numSpecifications = 0,
    .specifications = NULL,
